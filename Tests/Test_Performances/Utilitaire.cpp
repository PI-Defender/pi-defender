...
BOOLEAN IsServiceInstalled(_In_ LPCTSTR lpszServiceName)
/*
Description:
	Query if the service "lpszServiceName" is installed

Parameters:
	lpszServiceName - Name of the service

Return:
	TRUE - "lpszServieceName" is installed
	FALSE - "lpszServieceName" is not installed
*/
{
...
DWORD MyStartService(_In_ LPCTSTR lpszServiceName)
{
	SC_HANDLE hSCManager = NULL;
	SC_HANDLE hService = NULL;
	SERVICE_STATUS serviceStatus;
	DWORD dwWaitTime, dwResult = 0;
	...
	__try {
		...
		// Get Service handle, if the service is installed, we get the handle. Else we raise an error.
		hService = OpenService(hSCManager, lpszServiceName, SERVICE_START | SERVICE_QUERY_STATUS);
		if (!hService) {
			DisplayMessage(MESSAGE_TYPE::TYPE_ERROR, TEXT("%s is not installed."), lpszServiceName);
			__leave;
		}
		...
		if (serviceStatus.dwCurrentState == SERVICE_STOPPED || serviceStatus.dwCurrentState == SERVICE_STOP_PENDING) {
			...
			if (!StartService(hService, 0, NULL)) {
				DisplayError(TEXT("MyStartService"));
				__leave;
			}
		...
		dwResult = serviceStatus.dwCurrentState;
	}
...
DWORD MyStopService(_In_ LPCTSTR lpszServiceName)
{
	SC_HANDLE hSCManager = NULL;
	SC_HANDLE hService = NULL;
	SERVICE_STATUS serviceStatus;
	...
	__try {
		...
		// Get Service handle, if the service is installed, we get the handle. Else we raise an error.
		hService = OpenService(hSCManager, lpszServiceName, SERVICE_STOP | SERVICE_QUERY_STATUS);
		if (!hService) {
			DisplayMessage(MESSAGE_TYPE::TYPE_ERROR, TEXT("%s is not installed."), lpszServiceName);
			__leave;
		}
		...
		if (serviceStatus.dwCurrentState == SERVICE_RUNNING || serviceStatus.dwCurrentState == SERVICE_START_PENDING) {
			...
			if (!ControlService(hService, SERVICE_CONTROL_STOP, &serviceStatus)) {
				DisplayError(TEXT("ControlService"));
				__leave;
			}
		}
...
HANDLE GetRandomProcessHandle(_In_ DWORD dwDesiredAccess)
{
	HANDLE hProcess = NULL;
	HANDLE hSnapshot = NULL;
	BOOLEAN bResult = TRUE;
	...
	__try {
		...
		for (UINT i = 0; i < iRandomBuffer % iProcessCount; i++) {
			if (!Process32Next(hSnapshot, &ProcessEntry)) {
				DisplayError(TEXT("Process32Next"));
				__leave;
			}
		}
